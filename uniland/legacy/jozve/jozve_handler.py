...
from uniland.utils.filters import (
    document_submission, document_submission, document_search,
    document_check, document_react,document_submission_no
)
import pyrogram
...
async def document_handler(client:pyrogram.client.Client, message:pyrogram.types.messages_and_media.message.Message):
    await message.reply(
            text=messages.DOCUMENT_DESCRIPTION,
            reply_markup=pages.DOCUMENT
        )
@Client.on_message(filters.regex(messages.DOCUMENT_SEARCH))
async def document_search_handler(client:pyrogram.client.Client, message:pyrogram.types.messages_and_media.message.Message):
    await message.reply(
            text=messages.DOCUMENT_SEARCH_DESCRIPTION,
            reply_markup=pages.BACK
        )

@Client.on_message(filters.regex(messages.DOCUMENT_SUBMISSION))
async def document_submission_handler(client:pyrogram.client.Client, message:pyrogram.types.messages_and_media.message.Message):
    await message.reply(
...
async def document_submission_handler(client:pyrogram.client.Client, message:pyrogram.types.messages_and_media.message.Message):
    await message.reply(
            text=messages.DOCUMENT_REQUESTED_DESCRIPTION,
            reply_markup=pages.BACK
        )
@Client.on_message(document_submission)
async def document_submission_text_handler(client:pyrogram.client.Client, message:pyrogram.types.messages_and_media.message.Message):
    await methods.send_message_to_admin(client,message)
    await message.reply(
            text=messages.DOCUMENT_REQUESTED_CONFIRMATION,
            reply_markup=pages.HOME
        )
@Client.on_message(document_search)
async def document_search_text_handler(client:pyrogram.client.Client, message:pyrogram.types.messages_and_media.message.Message):
...
async def document_dl_handler(client:pyrogram.client.Client, message:pyrogram.types.messages_and_media.message.Message):
...
async def document_dl3_handler(client:pyrogram.client.Client, message:pyrogram.types.messages_and_media.message.Message):
    typed,_,id = message.data.split(':')
    add_like(typed,id)

@Client.on_callback_query(document_submission_no)
async def document_submission_no_handler(client:pyrogram.client.Client, message:pyrogram.types.messages_and_media.message.Message):
    print(message)
