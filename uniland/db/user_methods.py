...
def edit_admin_submission(admin_id: int, submission: Submission) -> bool:
  with USER_INSERTION_LOCK:
    ...
    admin = SESSION.query(User).filter(
      User.user_id == admin_id, User.access_level == UserLevel.Admin).first()
    if admin == None:
      SESSION.close()
      return False
    ...
    search_engine.index_record(submission.id, submission.search_text,
                               submission.submission_type, likes,
...
def count_active_users(minutes: int):
  return (SESSION.query(User).filter(User.last_active >= datetime.utcnow() -
                                     timedelta(minutes=minutes)).count())


def count_new_signups(minutes: int):
  return (SESSION.query(User).filter(User.signup_date >= datetime.utcnow() -
                                     timedelta(minutes=minutes)).count())
