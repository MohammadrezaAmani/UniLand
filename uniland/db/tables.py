...
class User(BASE):
    ...
    bookmarks = relationship(
        'Submission', secondary=bookmarks_association, back_populates='liked_users')
...
class Submission(BASE):
    ...
    # many-to-one
    correspondent_admin = Column(
        Integer, ForeignKey('users.user_id'), default=None)
    # many-to-one with User.user_submissions
    owner_id = Column(Integer, ForeignKey('users.user_id'), nullable=False)
    # many-to-many with User.bookmarks
    liked_users = relationship(
        'Submission', secondary=bookmarks_association, back_populates='bookmarks')

    faculty = Column(String(30), default='نامشخص')
    search_text = Column(String(200), default='')
    description = Column(
        String(500), default='توضیحاتی برای این فایل ثبت نشده است.')
...
# TODO! solve conflicts and build classes
# bookmarks_association.__table__.create(checkfirst=True)
