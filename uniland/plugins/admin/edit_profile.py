...
async def check_profile(client, message):
    global edited_staged_profs
    if message.from_user.id not in edited_staged_profs:
...
async def display_profile(client, message, profile):
    ...
    global edited_staged_profs
    profile = edited_staged_profs[message.from_user.id]
...
@Client.on_message(
    ...
    & user_step(UserSteps.EDIT_PROFILE_SUBMISSION_INPUT_TITLE.value)
)
async def start_getting_data(client, message):
    ...
    global edited_staged_profs
    edited_staged_profs[message.from_user.id] = profile
...
@Client.on_message(
    user_step(UserSteps.EDIT_PROFILE_SUBMISSION.value) & ~filters.me & filters.text
)
async def choose_doc_field(client, message):
    ...
    # Done Button
    if message.text.strip() == Triggers.EDIT_PROFILE_SUBMISSION_DONE.value:
        # User has finished the submission process
        profile = edited_staged_profs.pop(message.from_user.id)
        result = user_db.add_user_submission(message.from_user.id, profile)
        if result:
            await message.reply(text="پروفایل با موفقیت تغییر یافت. ")
        else:
            await message.reply(
    ...
    # Cancel Button
    elif message.text.strip() == Triggers.EDIT_PROFILE_SUBMISSION_CANCEL.value:
        # User has canceled the submission process
        await message.reply("عملیات ارسال فایل لغو شد.")
        edited_staged_profs.pop(message.from_user.id)
        await start_stage(client, message)
...
@Client.on_message(
    user_step(UserSteps.EDIT_PROFILE_SUBMISSION_EDIT_TITLE.value)
    & ~filters.me
    & filters.text
    & ~exact_match(Triggers.BACK.value)
)
async def update_profile_title(
    ...
    await display_profile(client, message, profile)
    await go_back(client, message, UserSteps.EDIT_PROFILE_SUBMISSION_EDIT_TITLE.value)
...
async def profile_phone(
    ...
    await display_profile(client, message, profile)
    await go_back(client, message, UserSteps.EDIT_PROFILE_SUBMISSION_PHONE.value)
...
async def profile_email(
    ...
    await display_profile(client, message, profile)
    await go_back(client, message, UserSteps.EDIT_PROFILE_SUBMISSION_EMAIL.value)
...
async def profile_university(
    ...
    await display_profile(client, message, profile)
    await go_back(client, message, UserSteps.EDIT_PROFILE_SUBMISSION_UNIVERSITY.value)
...
async def profile_faculty(
    ...
    await message.reply("عنوان اطلاعات با موفقیت تغییر کرد.")
    await display_profile(client, message, profile)
    await go_back(client, message, UserSteps.EDIT_PROFILE_SUBMISSION_FACULTY.value)
...
async def profile_owner_title(
    ...
    await display_profile(client, message, profile)
    await go_back(client, message, UserSteps.EDIT_PROFILE_SUBMISSION_OWNER_TITLE.value)
...
async def profile_description(
    ...
    await display_profile(client, message, profile)
    await go_back(client, message, UserSteps.EDIT_PROFILE_SUBMISSION_DESCRIPTION.value)
...
async def delete_profile_photo(client, message):
    if not await check_profile(client, message):
        return
    global edited_staged_profs
    if message.text == Triggers.EDIT_PROFILE_SUBMISSION_DELETE_PHOTO.value:
        # Deleting profile photo
        profile = edited_staged_profs[message.from_user.id]
        profile.image_id = ""
        await message.reply("عکس پروفایل با موفقیت حذف شد.")
        await display_profile(client, message, profile)
        await go_back(client, message, UserSteps.EDIT_PROFILE_SUBMISSION_PHOTO.value)
        return
...
async def profile_photo(client, message):
    ...
    global edited_staged_profs
    edited_staged_profs[message.from_user.id].image_id = image_id
    await message.reply("عکس پروفایل با موفقیت تغییر کرد.")
    await display_profile(client, message, edited_staged_profs[message.from_user.id])
    await go_back(client, message, UserSteps.EDIT_PROFILE_SUBMISSION_PHOTO.value)
...
async def profile_photo_document(client, message):
    ...
    if "image" in message.document.mime_type:
        ...
        await display_profile(
            client, message, edited_staged_profs[message.from_user.id]
        )
        await go_back(client, message, UserSteps.EDIT_PROFILE_SUBMISSION_PHOTO.value)
    else:
