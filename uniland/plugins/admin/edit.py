...
async def send_submission(client, message, submission):
  if submission:
    if submission.submission_type == "document":
      await message.reply_document(document=submission.file_id,
                                   caption=str(submission))
    elif submission.submission_type == 'profile':
      if submission.image_id == '':
        await message.reply_text(str(submission))
      else:
        await message.reply_document(document=submission.image_id,
                                     caption=str(submission))
...
async def start_editing_subs(
  ...
  user_db.update_user_step(message.from_user.id,
                           UserSteps.EDIT_SUBMISSION.value)
...
@Client.on_message(filters.text
                   & user_step(UserSteps.EDIT_DOCUMENT_SUBMISSION.value))
...
@Client.on_message(filters.text
                   & user_step(UserSteps.EDIT_PROFILE_SUBMISSION.value))
