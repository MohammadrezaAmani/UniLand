...
async def edit_access_level(client, message):
  ...
  await message.reply(text=Messages.ACCESS_LEVEL_BY_USERID.value, reply_markup=Pages.BACK)
...
async def get_access_level(client, message):
  ...
  await message.reply(text=output, reply_markup=Pages.ADMIN_PANEL_CHOOSE_NEW_ACCESS_LEVEL)
...
@Client.on_message(filters.text
                   ...
                   & access_level(min=3)
                   & (exact_match(Triggers.USER_ACCESS_LEVEL_ADMIN.value) | exact_match(Triggers.USER_ACCESS_LEVEL_EDITOR.value) | exact_match(Triggers.USER_ACCESS_LEVEL_BASIC.value)))
async def change_access_level(client, message):
  ...
  usercache.update_user_permission(user_id, new_access_level)
