...
async def edit_access_level(client, message):
    ...
    await message.reply(
        text=Messages.ACCESS_LEVEL_BY_USERID.value, reply_markup=Pages.BACK
    )
...
async def get_access_level(client, message):
    ...
    await message.reply(
        text=output, reply_markup=Pages.ADMIN_PANEL_CHOOSE_NEW_ACCESS_LEVEL
    )
...
async def change_access_level(client, message):
    ...
    user_db.update_user_access_level(user_id, new_access_level)  # update db & cache
